...
class RoIAlignFunction(Function):
    ...
    def forward(self, features, rois):
        ...
        batch_size, num_channels, data_height, data_width, data_length = features.size()
        num_rois = rois.size(0)

        output = features.new(num_rois, num_channels, self.aligned_height, self.aligned_width, self.aligned_length).zero_()
        if features.is_cuda:
    ...
    def backward(self, grad_output):
        ...
        batch_size, num_channels, data_height, data_width, data_length = self.feature_size

        grad_input = self.rois.new(batch_size, num_channels, data_height,
                                  data_width, data_length).zero_()
